...
void I2Cextender::init(int address, int chip, unsigned config) {
    ...
    case I2Cextender::ARDIO_B:    _size = B8;  inputB(_config);                        break;
    case I2Cextender::ARDIO_C:    _size = B8;  inputC(_config);                        break;
...
unsigned I2Cextender::read() {  
    ...
    switch (_chip) {
		...
		case I2Cextender::ARDIO_B:        
										  ...
										  bitWrite(data,6, digitalRead(A0));
										  bitWrite(data,7, digitalRead(A1));
																				break;
		case I2Cextender::ARDIO_C:        
										  bitWrite(data,0, digitalRead(A2));
										  bitWrite(data,1, digitalRead(A3));
#if defined(__AVR_ATmega32U4__)			// don't mess with I2C pins!
										  bitWrite(data,2, digitalRead(A4));
										  bitWrite(data,3, digitalRead(A5));
#endif
																				break;
...
void I2Cextender::inputB(int dir) {
	...
	pinMode(A0, bitRead(dir, 6) ? INPUT : OUTPUT);
	pinMode(A1, bitRead(dir, 7) ? INPUT : OUTPUT);
}
void I2Cextender::inputC(int dir) {
	pinMode(A2, bitRead(dir, 0) ? INPUT : OUTPUT);
	pinMode(A3, bitRead(dir, 1) ? INPUT : OUTPUT);
#if defined(__AVR_ATmega32U4__)		// don't mess with I2C pins!
	pinMode(A4, bitRead(dir, 2) ? INPUT : OUTPUT);
	pinMode(A5, bitRead(dir, 3) ? INPUT : OUTPUT);
#endif
}
